...
export class SubcategoryController {
  ...
  @ApiParam({
    name: 'lang',
    required: true,
    description: 'Мова запиту (uk або en)',
  })
  @ApiParam({
    name: 'subcategoryId',
    required: true,
    description: 'ID підкатегорії',
  })
  @ApiResponse({ status: 200, description: 'Підкатегорію успішно отримано' })
  @ApiResponse({
    status: 400,
    description: 'Помилка при отриманні підкатегорії',
  })
  ...
  async getSubcategory(
    ...
    try {
      this.logger.log(
        `Отримання підкатегорії ID: ${subcategoryId} для мови: ${lang}`,
      );
      if (lang === 'uk') {
        return await this.subcategoryService.getUkSubcategoryById(
          subcategoryId,
        );
      } else if (lang === 'en') {
        return await this.subcategoryService.getEnSubcategoryById(
          subcategoryId,
        );
      } else {
        throw new BadRequestException(
          'Непідтримувана мова. Використовуйте "uk" або "en".',
        );
      }
    } catch (error) {
  ...
  @ApiResponse({
    status: 400,
    description: 'Помилка при створенні підкатегорії',
  })
  @ApiBody({
    type: CreateSubcategoryDto,
    description: 'Дані для створення підкатегорії',
  })
  async create(
    ...
    try {
      this.logger.log(
        `Створення підкатегорії для категорії ID: ${categoryId}`,
        JSON.stringify(createSubcategoryDto),
      );
      return await this.subcategoryService.create(
        categoryId,
  ...
  @ApiResponse({
    status: 400,
    description: 'Помилка при оновленні підкатегорії',
  })
  @ApiResponse({ status: 404, description: 'Підкатегорія не знайдена' })
  @ApiBody({
    type: UpdateSubcategoryDto,
    description: 'Дані для оновлення підкатегорії',
  })
  async update(
    @Param('subcategoryId', ParseIntPipe) subcategoryId: number,
    ...
    try {
      this.logger.log(
        `Оновлення підкатегорії ID: ${subcategoryId}`,
        JSON.stringify(updateSubcategoryDto),
      );
  ...
  @ApiResponse({
    status: 400,
    description: 'Помилка при видаленні підкатегорії',
  })
  @ApiResponse({ status: 404, description: 'Підкатегорія не знайдена' })
  async delete(@Param('subcategoryId', ParseIntPipe) subcategoryId: number) {
    try {
      ...
      return {
        message: `Підкатегорію з ID ${subcategoryId} успішно видалено.`,
      };
    } catch (error) {
